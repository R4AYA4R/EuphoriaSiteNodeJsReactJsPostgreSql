...
const UserPageFormComponent = () => {
    ...
    const registration = async (email: string, password: string) => {
        ...
        try {
            ...
            let name = inputSignUpName.trim(); // помещаем в переменную name(указываем ей именно let,чтобы можно было изменять) значение инпута имени,отфильтрованное по пробелам,если его не отфильтровать здесь по пробелам,то даже при следующем изменении этой переменной name,если там указать trim(),то не будет работать,и не будет заменять первую букву на заглавную

            name = name.replace(name[0], name[0].toUpperCase());  // заменяем первую букву этой строки инпута имени(name[0] в данном случае) на первую букву этой строки инпута имени только в верхнем регистре(name[0].toUpperCase()),чтобы имя начиналось с большой буквы,даже если написали с маленькой
