...
const ProductItemPage = () => {
    ...
    const { mutate } = useMutation({
        ...
        onSuccess() {
            ...
            refetchCommentsArrivals(); // также переобновляем массив всех комментариев для секции sectionNewArrivals,чтобы там сразу переобновилось количество этих комментариев после добавления нового комментария,а не после обновления страницы,также этот повторный запрос на переобновление всех комментариев для секции sectionNewArrivals можно было указать в onSuccess для мутации рейтинга(то есть в функции запроса на сервер на обновление рейтинга товара),там тоже бы работало,но тогда мог бы делаться дополнительный запрос на сервер на переобновление всех комментариев для секции sectionNewArrivals при обновлении страницы и изменении основного массива комментариев для товара,а не только после добавления нового комментария,поэтому лучше указать это здесь
    ...
    const { mutate: mutateProductRating } = useMutation({
        ...
        onSuccess() {
            ...
            // refetchCommentsArrivals(); // не переобновляем здесь массив всех комментариев для секции sectionNewArrivals,так как тогда мог бы делаться дополнительный запрос на сервер на переобновление всех комментариев для секции sectionNewArrivals при обновлении страницы и изменении основного массива комментариев для товара,а не только после добавления нового комментария,поэтому лучше указать этот запрос на сервер на переобновление всех комментариев для секции sectionNewArrivals в onSuccess у функции запроса на сервер для создания нового комментария,поэтому этот код закомментировали
    ...
    const submitFormHandler = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (textAreaValue.trim().length <= 10 || textAreaValue.trim().length > 300) {
            ...
            // здесь надо будет потом adminReply делать null
            mutate({ name: user.userName, text: textAreaValue, rating: activeStarsForm, createdTime: showTime, productId: data?.data.id } as IComment); // вызываем функцию post запроса на сервер,создавая комментарий,разворачивая в объект нужные поля для комментария и давая этому объекту тип as IComment,чтобы не выдавало ошибку,что для productId нельзя назначить undefined значение,а также чтобы не указывало ошибку,что не указали поле id(вручную не указываем id,чтобы он автоматически создавался на сервере), указываем поле productId со значением как у id товара на этой странице(data?.data.id),чтобы в базе данных связать этот товар с комментарием,поле adminReply просто не указываем,чтобы изначально комментарий создавался без ответа от админа,то есть поле adminReply в базе данных будет null,так как мы его не указываем,или можно было тогда в типе IComment указать полю adminReply,что оно может быть еще и null,тогда можно было его указать просто со значением null типа adminReply:null, но в данном случае его просто не указываем
    ...
    // если isLoading true и isLoadingUser true,то есть идет первоначальная загрузка комментариев товара и идет загрузка данных пользователя(указываем эту проверку,чтобы показывался лоадер,пока еще данные пользователя загружаются,чтобы правильно показывался текст потом,что товар в корзине),если в этой проверке вместо &&(и) указать ||(или),то будет идти бесконечный цикл запросов на сервер,поэтому указываем &&(и),то показывать лоадер,иначе,если это не отслеживать и не показывать,то комментарии,и число комментариев будет появляться через время
