...
class CommentController {
    ...
    async getCommentsForProduct(req, res, next) {
        ...
        let { productId, page, limit } = req.query; // берем из url(строки) запроса параметр productNameFor,чтобы получить все комментарии для конкретного товара, указываем здесь let,чтобы можно было изменять значения этих параметров(переменных),в данном случае это надо для limit и page,а также,чтобы не выдавало ошибку,когда не передаем эти параметры,что эти параметры undefined,так как будем использовать этот эндпоинт и для получения просто всех комментариев для секции sectionNewArrivals на фронтенде
    ...
    async addReplyForComment(req, res, next) {
        ...
        try {
            ...
            const updatedComment = await commentService.addReplyForComment(comment);  // вызываем нашу функцию addReplyForComment из commentService,передаем туда параметр comment(объект тела запроса),эта функция возвращает измененный объект комментария и помещаем его в переменную updatedComment

            return res.json(updatedComment); // возвращаем на клиент измененный объект комментария
    ...
    async deleteReplyFromAdmin(req, res, next) {
        ...
        const { commentId } = req.params; // берем(деструктуризируем) из параметров запроса поле commentId(id объекта комментария,у которого хотим удалить ответ от админа)

        // оборачиваем в try catch для обработки ошибок
        try {

            const updatedComment = await commentService.deleteReplyFromAdmin(commentId);  // вызываем нашу функцию deleteReplyFromAdmin из commentService,передаем туда параметр commentId(id объекта комментария,у которого хотим удалить ответ от админа),эта функция возвращает измененный объект комментария и помещаем его в переменную updatedComment
    ...
    async deleteComment(req, res, next) {
        ...
        try {
            ...
            const deletedCommentData = await commentService.deleteComment(commentId);  // вызываем нашу функцию deleteComment из commentService,передаем туда параметр commentId(id объекта комментария,который хотим удалить),эта функция возвращает 1(количество удаленных записей(объектов) из базы данных,то есть если был успешно удален объект комментария)или вернет 0,если не был удален объект комментария

            return res.json(deletedCommentData); // возвращаем на клиент deletedCommentData возвращает 1(количество удаленных записей(объектов) из базы данных,то есть если был успешно удален объект комментария)или вернет 0,если не был удален объект комментария
