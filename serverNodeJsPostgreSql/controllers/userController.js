...
class UserController {
    ...
    async changeAccInfo(req, res, next) {
        ...
        try {
            ...
            const newUserData = await userService.changeAccInfo(userId, name, email, errors); // вызываем нашу функцию changeAccInfo в userService и туда передаем параметры и в переменную newUserData помещаем новый измененный объект пользователя в базе данных,передаем параметр errors в нашу функцию changeAccInfo,чтобы там обработать ошибку при валидации поля email
    ...
    async changeAccPass(req, res, next) {
        ...
        try {
            ...
            const newUserData = await userService.changeAccPass(userId, currentPass, newPass); // вызываем нашу функцию changeAccPass в userService и туда передаем параметры и в переменную newUserData помещаем новый измененный объект пользователя в базе данных
