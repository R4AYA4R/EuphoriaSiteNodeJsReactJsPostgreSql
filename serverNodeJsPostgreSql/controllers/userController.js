...
export default new UserController(); // экспортируем объект на основе класса UserController,чтобы потом сразу можно было после импорта этого объекта из этого файла указывать через точку функции этого класса UserController