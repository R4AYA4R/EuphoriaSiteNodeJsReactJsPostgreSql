...
class ProductController {
    ...
    // функция для удаления картинки описания товара каталога
    async deleteDescImage(req, res, next) {

        // отличие req.params от req.query заключается в том,что в req.params указываются параметры в самом url до эндпоинта на бэкэнде(в node js в данном случае,типа /api/getProductById) через :(типа /:id,динамический параметр id),а req.query - это параметры,которые берутся из url(которые дополнительно добавили с фронтенда к url) через знак ?(типа ?name=bob)

        const { productId, imageName } = req.params; // берем(деструктуризируем) из параметров запроса поле productId и imageName

        // оборачиваем в try catch для обработки ошибок
        try {

            const updatedProductData = await productService.deleteDescImage(productId, imageName);  // вызываем нашу функцию deleteDescImage из productService,передаем туда параметры productId и imageName,эта функция для удаления картинки описания с сервера(из папки static в данном случае) и у объекта товара в базе данных и возвращает объект с объектом обновленного товара в базе данных,если он был найден и типа строку с сообщением

            return res.json(updatedProductData); // возвращаем на клиент объект с полями измененного объекта товара каталога,если он был найден и типа строку с сообщением,возвращаем именно объект,так как уже указываем несколько полей в этом объекте,если не вернуть объект для нескольких полей,то выдает ошибку
