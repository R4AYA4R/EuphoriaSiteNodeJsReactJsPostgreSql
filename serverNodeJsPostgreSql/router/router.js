...
router.post('/uploadFile', authMIddleware, fileController.uploadFile); // указываем post запрос для загрузки файла с фронтенда на наш node js сервер(в данном случае в папку static),вторым параметром указываем наш authMiddleware для проверки на access токен

router.delete('/deleteImage/:imageName', fileController.deleteImage); // указываем delete запрос для удаления файла с нашего node js сервера(в данном случае из папки checkStatic),delete запрос не имеет тела запроса и все параметры передаются через строку,тут указываем через :(двоеточие) динамический параметр imageName,то есть этот параметр может меняться(в данном случае этот параметр нужен,чтобы удалить файл из папки checkStatic по этому названию imageName)

router.delete('/deleteCheckStatic', fileController.deleteCheckStatic);  // указываем delete запрос,чтобы удалить все файлы из папки checkStatic,чтобы можно было опять туда загружать повторные файлы(то есть когда пользователь(админ) только выбирает картинки для товара в форме создания товара,но еще их не сохранил для товара,чтобы потом можно было нормально работать с загрузкой этих картинок,потом будем удалять файлы из этой папки checkStatic,при запуске страницы,чтобы если вдруг админ выбрал картинку,она уже загрузилась в эту папку,но потом админ не сохранил этот товар,а обновил страницу,то чтобы он заново смог загрузить эту картинку,иначе выдавалась бы ошибка,что такой файл уже существует),не указываем здесь authMiddleware,так как если делать проверку на авторизован ли пользователь,то не будет вовремя удаляться эта папка,так как когда пользователь авторизовывается,то обновление страницы не происходит,а мы удаляем эту папку только при запуске(или обновлении) страницы 
